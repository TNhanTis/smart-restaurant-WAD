...
if (!VNPAY_RETURN_URL) {
  ...
  console.log('Ví dụ:');
  console.log(
    '  node test-vnpay-return.js "http://localhost:5173/payment/result?vnp_Amount=10150220&vnp_BankCode=NCB&..."',
  );
  console.log('');
  console.log('Hoặc nhập query string:');
  console.log(
    '  node test-vnpay-return.js "vnp_Amount=10150220&vnp_BankCode=NCB&..."',
  );
  process.exit(1);
}
...
async function testVNPayReturn() {
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/payments/vnpay/return?${queryString}`,
    );
    ...
    if (response.data.success) {
      ...
      console.log(
        '   Amount:',
        (response.data.amount || 0).toLocaleString(),
        'VND',
      );
    } else {
